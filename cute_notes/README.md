...
- [py-mlir-jit.md](cutedsl/py-mlir-jit.md) — Argument marshalling protocol between Python objects and MLIR SSA values for JIT function boundaries
- [loop.md](cutedsl/loop.md) — `range`, `range_constexpr`, unrolling (`unroll`, `unroll_full`), software pipelining (`prefetch_stages`)
- [constexpr.md](cutedsl/constexpr.md) — `Constexpr` type annotation, `const_expr()`, calling conventions (`@jit`/`@kernel`/Python), auto-constexpr rules
- [constexpr_typing_PR.md](cutedsl/constexpr_typing_PR.md) — RFC: making `Constexpr` compatible with Python type checkers (`Annotated` vs `TYPE_CHECKING` approaches)
- [inline_ptx.md](cutedsl/inline_ptx.md) — `llvm.inline_asm` API for emitting PTX directly, bypassing MLIR/LLVM IR optimization
- [experiments.md](cutedsl/experiments.md) — Environment setup, PTX/CUBIN/SASS dump, trace-time vs runtime, kernel templates
